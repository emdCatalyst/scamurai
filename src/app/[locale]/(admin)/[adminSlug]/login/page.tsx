...
export default function AdminLogin() {
  ...
  // Silently remove all active sessions without triggering Clerk's
  // post-signOut navigation (which defaults to "/" and bounces us off the
  // admin login page mid-flow).
  ...
  // Stays true from post-signIn until the browser navigates away, masking the
  // Clerk session-cookie propagation gap so the user doesn't see a 404 from
  // the authenticated layout rendering before cookies are usable server-side.
  const [isRedirecting, setIsRedirecting] = useState(false);

  // Smart redirect: if already logged in as master_admin, skip login page.
  // We skip this once a submit has kicked off — the form-submit path handles
  // its own navigation, and we don't want this effect to race it with a
  // router.replace that hits the authenticated layout before cookies settle.
  useEffect(() => {
  ...
  }, [clerkUser, isUserLoaded, locale, adminSlug, router, isRedirecting, isLoading]);
  ...
  const onSubmit = async (data: LoginFormValues) => {
    ...
    try {
      await clearActiveSessions(); // Clear any existing sessions silently
      ...
      if (result.status === "complete") {
        ...
        if (verification.success) {
          // Hard navigate. router.push hits the authenticated server layout
          // before the new Clerk session cookie is fully readable, which
          // makes that layout call notFound() — visible to the user as a 404
          // until they refresh. window.location.assign forces a fresh request
          // with the cookie in place, eliminating the race.
          setIsRedirecting(true);
          window.location.assign(`/${locale}/${adminSlug}/dashboard`);
          return;
        } else {
          // Wrong role — drop the just-created session but stay on this page
          // so the user sees the access-denied message.
  ...
  return (
    <div
      className={`relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-hero p-6 ${isAr ? "font-arabic" : "font-sans"}`}
      dir={isAr ? "rtl" : "ltr"}
    >
      {isRedirecting && (
        <div
          className="fixed inset-0 z-[200] flex flex-col items-center justify-center gap-4 bg-navy/95 backdrop-blur-sm"
          role="status"
          aria-live="polite"
        >
          <Loader2 className="animate-spin text-sky" size={36} strokeWidth={1.5} />
