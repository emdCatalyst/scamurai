...
export default function DeliveryAppsError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('[admin/delivery-apps] error:', error);
  }, [error]);
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 bg-red-50/50 rounded-2xl border border-red-100 shadow-sm px-6 text-center">
      ...
      <p className="text-slate-600 max-w-md mb-8">
        We encountered an error while loading delivery apps. This has been logged, and you can try refreshing.
      </p>
      <button
        onClick={() => reset()}
