...
interface UsersTableProps {
  users: UserRowType[];
  onDetailOpen: (user: UserRowType) => void;
}

export function UsersTable({ users, onDetailOpen }: UsersTableProps) {
  ...
  if (users.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-white rounded-xl border border-slate-100 shadow-sm">
        ...
        <p className="text-slate-500 text-sm leading-relaxed">{t('emptyDesc')}</p>
      </div>
    );
  }
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100">
              <th className="py-4 px-4 text-xs font-bold uppercase tracking-wider text-slate-500 ps-6">{t('user')}</th>
              <th className="py-4 px-4 text-xs font-bold uppercase tracking-wider text-slate-500">{t('role')}</th>
              <th className="py-4 px-4 text-xs font-bold uppercase tracking-wider text-slate-500">{t('brand')}</th>
              <th className="py-4 px-4 text-xs font-bold uppercase tracking-wider text-slate-500">{t('branch')}</th>
              <th className="py-4 px-4 text-xs font-bold uppercase tracking-wider text-slate-500">{t('joined')}</th>
